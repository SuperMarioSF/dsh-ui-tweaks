...
.dsh-ui-subagent-aggregate-table{width:100%;table-layout:fixed;margin:0}
.dsh-ui-subagent-aggregate-table th{padding:2px 3px!important;text-align:left;background:var(--dsw-alias-bg-layer-1)}
...
`;
		...
		function activateNestedSubagentSticky(childrenContainer, aggregateTop, containerTop) {
			...
			for (const item of items) {
				const detail = item.querySelector(":scope > .dsh-ui-subagent-detail");
				if (!detail) continue;
				...
				if (itemRect.top <= containerTop + aggregateTop && detailRect.bottom > containerTop + aggregateTop + SUBAGENT_STICKY_HEADER_H) {
					const parentDetail = childrenContainer.closest(".dsh-ui-subagent-detail");
					...
					const aggregateHeader = item.querySelector(":scope > .dsh-ui-subagent-aggregate-table > thead.dsh-ui-subagent-sticky-header");
					if (aggregateHeader) {
		...
		function BalanceBadge({ sessionId, useSession }) {
			...
			const renderSubagentBlock = (node) => {
				...
				return React.createElement(
					...
					React.createElement(
						"table",
						{ className: "dsh-ui-request-table dsh-ui-subagent-request-table dsh-ui-subagent-aggregate-table" },
						React.createElement(
							"thead",
							{ className: "dsh-ui-subagent-sticky-header" },
							React.createElement(
