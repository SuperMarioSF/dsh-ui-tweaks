...
.dsh-ui-request-section{margin-top:10px;border-top:1px solid var(--dsw-alias-border-l1);max-height:40vh;overflow-y:auto}
...
.dsh-ui-request-table th:nth-child(4),.dsh-ui-request-table td:nth-child(4),.dsh-ui-request-table th:nth-child(5),.dsh-ui-request-table td:nth-child(5),.dsh-ui-request-table th:nth-child(6),.dsh-ui-request-table td:nth-child(6){width:90px}
.dsh-ui-request-table th:last-child,.dsh-ui-request-table td:last-child{width:80px}
.dsh-ui-subagent-toggle{height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:0;border-radius:6px;padding:0 4px;font-size:12px;white-space:nowrap}
.dsh-ui-subagent-toggle:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}
.dsh-ui-subagent-cell{padding:0!important;border-bottom:0!important;background:var(--dsw-alias-bg-layer-1)}
.dsh-ui-subagent-box{padding:8px;border-top:1px solid var(--dsw-alias-border-l1)}
.dsh-ui-subagent-item{display:flex;align-items:center;gap:8px;padding:4px 6px;border-radius:6px}
.dsh-ui-subagent-item:hover{background:var(--dsw-alias-interactive-bg-hover)}
.dsh-ui-subagent-item-name{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary)}
.dsh-ui-subagent-item-cost{color:var(--dsw-alias-label-secondary);white-space:nowrap}
.dsh-ui-subagent-table{margin:6px 0 4px;background:var(--dsw-alias-bg-layer-2);border-radius:8px}
...
`;
		...
		function computeRequestUsage(requests) {
			...
			for (const request of requests) {
				...
				rows.push({
					key: `${request.startSeq}-${rows.length}`,
		...
		function mergeRequestRows(...lists) {
			...
			for (const list of lists) {
				for (const row of list) {
					if (row.seq != null && !map.has(row.seq)) map.set(row.seq, row);
				}
			}
		...
		function BalanceBadge({ sessionId, useSession }) {
			...
			const requestRows = detailRows.length > 0 ? detailRows : windowRequestRows;
			...
			const [subagentExpanded, setSubagentExpanded] = React.useState(false);
			...
			const displayUsageRows = React.useMemo(() => {
				...
				const row = {
					...
					uncachedCost: 0,
					outputCost: 0,
					cost: subagentTotalCost
				};
				for (const subagent of subagentState.subagents) {
					for (const usageRow of subagent.rows) {
						row.cacheInput += usageRow.cacheInput;
						row.uncachedInput += usageRow.uncachedInput;
						row.output += usageRow.output;
						row.cacheCost += usageRow.cacheCost;
						row.uncachedCost += usageRow.uncachedCost;
						row.outputCost += usageRow.outputCost;
					}
				}
				return [...usageRows, row];
			}, [usageRows, subagentState.subagents, subagentTotalCost]);
			...
			const loadOlder = React.useCallback(async () => {
				...
				const oldestSeq = detailRows.length ? Math.min(...detailRows.map((row) => row.seq ?? Infinity)) : undefined;
			...
			return React.createElement(
				...
				open && React.createElement(
					...
					displayUsageRows.length === 0
						...
						: React.createElement(
							...
							React.createElement(
								"tbody",
								null,
								displayUsageRows.map((row) => row.period === "all"
									? React.createElement(
										"tr",
										{ key: "dsh-ui-subagent-total" },
										React.createElement(
											"td",
											null,
											React.createElement(
												...
												{
													type: "button",
													className: "dsh-ui-subagent-toggle",
													onClick: () => {
														if (subagentState.status !== "ready") loadSubagents();
														setSubagentExpanded((value) => !value);
													}
												},
												subagentExpanded ? "⮝(收起) 子代理" : "⮟(展开) 子代理"
											)
										),
								...
								subagentExpanded && subagentState.subagents.length > 0 && React.createElement(
									"tr",
									{ key: "dsh-ui-subagent-expand" },
									React.createElement(
										"td",
										{ colSpan: 6, className: "dsh-ui-subagent-cell" },
										React.createElement(
											"div",
											{ className: "dsh-ui-subagent-box" },
											subagentState.subagents.map((subagent) => React.createElement(
												"div",
												{ key: subagent.id, className: "dsh-ui-subagent-item" },
												React.createElement(
													"button",
													{
														type: "button",
													...
													expandedSubagentIds.has(subagent.id) ? "⮝(收起)" : "⮟(展开)"
												),
												React.createElement("span", { className: "dsh-ui-subagent-item-name", title: subagent.id }, `${subagent.label}${subagent.depth > 1 ? ` (d${subagent.depth})` : ""}`),
												React.createElement("span", { className: "dsh-ui-subagent-item-cost" }, formatCost(subagent.totalCost)),
												expandedSubagentIds.has(subagent.id) && React.createElement(
													"table",
													{ className: "dsh-ui-usage-table dsh-ui-subagent-table" },
													React.createElement(
														"thead",
														null,
														React.createElement(
													...
													React.createElement(
														"tbody",
														null,
														subagent.rows.map((usageRow) => React.createElement(
															"tr",
															{ key: `${usageRow.model}:${usageRow.period}` },
															React.createElement("td", null, usageRow.model),
															React.createElement("td", null, usageRow.period === "peak" ? React.createElement("span", { className: "dsh-ui-peak" }, "高峰") : React.createElement("span", { className: "dsh-ui-offpeak" }, "闲时")),
															React.createElement("td", null,
																React.createElement("div", null, formatTokens(usageRow.cacheInput)),
																React.createElement("div", { className: "dsh-ui-usage-subcost" }, formatCost(usageRow.cacheCost))
															),
															React.createElement("td", null,
																React.createElement("div", null, formatTokens(usageRow.uncachedInput)),
																React.createElement("div", { className: "dsh-ui-usage-subcost" }, formatCost(usageRow.uncachedCost))
															),
															React.createElement("td", null,
																React.createElement("div", null, formatTokens(usageRow.output)),
																React.createElement("div", { className: "dsh-ui-usage-subcost" }, formatCost(usageRow.outputCost))
															),
															React.createElement("td", null, formatCost(usageRow.cost))
														))
													)
												)
											))
					...
					showRequests && requestRows.length > 0 && React.createElement(
						"div",
						...
						React.createElement(
							...
							React.createElement(
								"tbody",
								null,
								requestRows.map((row) => React.createElement(
									"tr",
									{ key: row.key },
									React.createElement("td", null, new Date(row.time).toLocaleString("zh-CN", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false })),
