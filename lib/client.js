...
`;
		...
		function historyEventsToRequests(events, headerEpochs) {
			...
			for (const event of events) {
				if (event.type === "request/header") {
					...
					if (compactionId != null) compactionStartSeqs.set(compactionId, event.seq);
					...
					requests.push({
						startSeq: compactionStartSeqs.get(event.data?.compactionId) ?? event.seq,
						startedAt: event.time,
						completedAt: event.time,
						provenance: provider !== void 0 || model !== void 0 ? { provider, model } : undefined,
		...
		function BalanceBadge({ sessionId, useSession }) {
			...
			const [historyState, setHistoryState] = React.useState({ status: "loading", rows: [], lastSeq: 0, seenSeqs: [], incomplete: true });
			...
			React.useEffect(() => {
				let cancelled = false;
				setHistoryState({ status: "loading", rows: [], lastSeq: 0, seenSeqs: [], incomplete: true });
				fetchAllSessionHistory(sessionId)
					.then(({ events, incomplete, headerEpochs }) => {
						...
						setHistoryState({
							...
							lastSeq,
							seenSeqs: historyRequests.map((request) => request.startSeq ?? 0),
							incomplete
						});
					})
					.catch(() => {
						if (!cancelled) setHistoryState({ status: "error", rows: [], lastSeq: 0, seenSeqs: [], incomplete: true });
					});
				return () => {
					cancelled = true;
				};
			...
			React.useEffect(() => {
				...
				const seen = new Set(historyState.seenSeqs);
				const seenInBatch = new Set();
				const newRequests = [];
				for (const request of requests ?? []) {
					const seq = request.startSeq ?? 0;
					if (seq === 0 || seen.has(seq) || seenInBatch.has(seq)) continue;
				...
				console.log("[dsh-ui-tweaks] diff " + JSON.stringify({
					requests: newRequests.map((r) => ({ seq: r.startSeq, usage: r.usage })),
					rows: newRows
				}));
				setHistoryState((prev) => ({
					...
					seenSeqs: [...prev.seenSeqs, ...newRequests.map((request) => request.startSeq ?? 0)]
				}));
				setDetailRows((prev) => mergeRequestRows(prev, newDetails));
			}, [requests, historyState.status, historyState.seenSeqs, historyState.lastSeq]);
