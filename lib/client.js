...
.dsh-ui-request-subagent-detail-row.dsh-ui-sticky-active{position:sticky;top:22px;z-index:3}
...
.dsh-ui-subagent-detail.dsh-ui-sticky-active{position:sticky;z-index:2}
		...
		function activateNestedSubagentSticky(childrenContainer, aggregateTop, containerTop) {
			...
			for (const item of items) {
				...
				if (itemRect.top <= containerTop + aggregateTop && detailRect.bottom > containerTop + aggregateTop + SUBAGENT_STICKY_HEADER_H) {
					...
					detail.classList.add("dsh-ui-sticky-active");
					detail.style.top = `${aggregateTop + SUBAGENT_STICKY_HEADER_H}px`;
