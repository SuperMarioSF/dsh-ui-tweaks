...
`;
		...
		function BalanceBadge({ sessionId, useSession }) {
			...
			const totalCost = React.useMemo(() => usageRows.reduce((sum, row) => sum + row.cost, 0), [usageRows]);
			...
			const refresh = React.useCallback(async () => {
				try {
					...
					if (data && Array.isArray(data.balance_infos) && data.balance_infos.length > 0) {
						...
						setState({ status: "ok", text: `余额 ${currency}${total}` });
					} else if (data && data.error === "missing-key") {
						setState({ status: "error", text: "未配置密钥" });
					} else {
						setState({ status: "error", text: "余额不可用" });
					}
				} catch {
					setState({ status: "error", text: "余额不可用" });
				}
			...
			React.useEffect(() => {
				if (turnCount > lastTurnRef.current) refresh();
				lastTurnRef.current = turnCount;
			}, [turnCount, refresh]);
					...
					usageRows.length === 0
						...
						: React.createElement(
							...
							React.createElement(
								"tbody",
								null,
								usageRows.map((row) => React.createElement(
									"tr",
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								...
								usageRows.length > 0 ? `预估费用 ${formatCost(totalCost)}` : "无价格信息"
							),
						...
						React.createElement(
							...
							React.createElement(
								"button",
								{ type: "button", className: "dsh-ui-balance-popup-refresh", onClick: refresh },
								"刷新余额"
							)
						)
					),
