...
.dsh-ui-subagent-tag{display:inline-block;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:bottom;color:var(--dsw-alias-label-secondary)}
...
`;
		...
		function BalanceBadge({ sessionId, useSession }) {
			...
			const subagentRequestRows = React.useMemo(() => {
				const rows = [];
				for (const subagent of subagentState.subagents) {
					const requests = (subagent.requests ?? []).map((request) => ({
						...request,
						subagentId: subagent.id,
						subagentLabel: subagent.label
					}));
					rows.push(...computeRequestUsage(requests));
				}
				return rows;
			...
			const requestRows = React.useMemo(() => mergeRequestRows(detailRows.length > 0 ? detailRows : windowRequestRows, subagentRequestRows), [detailRows, windowRequestRows, subagentRequestRows]);
						...
						React.createElement(
							...
							React.createElement(
								...
								requestRows.map((row) => React.createElement(
									...
									React.createElement("td", null, row.subagentLabel
										? React.createElement("span", { className: "dsh-ui-subagent-tag", title: `${row.subagentLabel} · ${row.model}` }, `子代理 ${row.subagentLabel} · ${row.model}`)
										: (row.purpose === "compaction" ? `(压缩) ${row.model}` : row.model)),
									React.createElement("td", null, row.period === "peak" ? React.createElement("span", { className: "dsh-ui-peak" }, "高峰") : React.createElement("span", { className: "dsh-ui-offpeak" }, "闲时")),
									React.createElement("td", null,
										React.createElement("div", null, formatTokens(row.cacheInput)),
										React.createElement("div", { className: "dsh-ui-usage-subcost" }, formatCost(row.cacheCost))
